...
const S={projects:[],activeId:null,editingId:null,detailId:null,mode:'hub'};
...
function createRuntimeProject(meta){
  return {
    id:String(meta.id),name:String(meta.name),note:String(meta.note||''),createdAt:Number(meta.createdAt)||Date.now(),
    status:'ยังไม่มีไฟล์ · ต้องเลือกไฟล์ใหม่หลัง Refresh',metrics:{files:0,years:[],plots:0,dataFiles:[]},attachments:[],
    iframe:null,observer:null,resizeObserver:null,statusTimer:null
  };
}
...
function frameMetrics(p){
  ...
  try{
    ...
    const dataFiles=cards.map(card=>({
      name:card.querySelector('.file-card-main strong')?.textContent?.trim()||'Excel',
      meta:card.querySelector('.file-card-main small')?.textContent?.trim()||'',
      years:[...card.querySelectorAll('.file-tags span')].map(x=>x.textContent.trim())
    }));
  ...
  }catch(err){console.warn('Unable to read project metrics',err);return base}
}
function projectStatus(p){
  const m=p.metrics||frameMetrics(p);
  const parts=[];
  if(m.files)parts.push(`${m.files} ไฟล์`,`${m.years.length} ปีงาน`,`${m.plots} แปลงก่อนรวมซ้ำ`);
  else parts.push('ยังไม่มีไฟล์ Progress');
...
function renderSummary(){
  ...
  $('activeProjectSummary').innerHTML=p?`โครงการที่กำลังดู<br><b>${esc(p.name)}</b>`:`เลือกโครงการก่อนดู Progress<br><b>${S.projects.length} โครงการในรายการ</b>`;
}
function renderCards(){
  const container=$('projectCards');
  if(!S.projects.length){
    container.innerHTML=`<button class="project-add-card empty-project-add" type="button" data-action="add"><span>＋</span><strong>สร้างโครงการแรก</strong><small>ตั้งชื่อโครงการก่อนเพิ่ม Excel และเอกสาร</small></button>`;
    return;
  }
  const cards=[...S.projects].sort((a,b)=>a.createdAt-b.createdAt).map(p=>{
    const m=p.metrics||frameMetrics(p);
    const badges=[m.years.length?`${m.years.length} ปีงาน`:null,m.plots?`${m.plots} แปลง`:null,p.attachments.length?`${p.attachments.length} เอกสาร`:null].filter(Boolean);
    return `<article class="project-card ${p.id===S.activeId?'active':''}" data-project="${esc(p.id)}">
      ...
      <div class="project-card-facts">${badges.length?badges.map(x=>`<span>${esc(x)}</span>`).join(''):'<span>รอเพิ่มไฟล์ Progress</span>'}</div>
      <div class="project-status"><i></i><span>${esc(projectStatus(p))}</span></div>
      <button type="button" class="project-open-btn" data-action="open" data-project="${esc(p.id)}">ดูรายละเอียดโครงการ</button>
    </article>`;
  }).join('');
...
function renderWorkspaceHead(){
  ...
  $('workspaceProjectNote').textContent=p.note||'Dashboard และไฟล์ของโครงการนี้แยกจากโครงการอื่น';
  $('workspaceStatus').textContent=projectStatus(p);
}
...
function setMode(mode){
  ...
  if(mode==='workspace'){
    const p=currentProject();
    if(!p){showHub();return}
    showOnlyFrame(p.id);renderWorkspaceHead();requestAnimationFrame(()=>syncFrameHeight(p));
  }
  renderAll();
}
...
function selectProject(id){
  ...
  S.activeId=id;ensureFrame(p);closeProjectDetail();setMode('workspace');document.title=`${p.name} · Work Progress Monitor`;
}
...
function ensureFrame(p){
  ...
  frame.className='project-iframe';frame.title=`Dashboard: ${p.name}`;frame.src=frameUrl(p);frame.loading='eager';frame.dataset.project=p.id;frame.hidden=true;
  $('frameStack').appendChild(frame);p.iframe=frame;
...
function updateProjectStatus(p){
  if(!p?.iframe)return;
  p.metrics=frameMetrics(p);p.status=projectStatus(p);
  renderCards();if(p.id===S.activeId)renderWorkspaceHead();if(p.id===S.detailId&&!$('projectDetailModal').hidden)renderProjectDetail();syncFrameHeight(p);
}
...
function attachFrameObservers(p){
  try{
    const doc=p.iframe.contentDocument;if(!doc?.body)return;
...
function renderProjectDetail(){
  const p=detailProject();if(!p)return;
  p.metrics=frameMetrics(p);
  const m=p.metrics;
  ...
  $('projectDetailContent').innerHTML=`
    <header class="project-detail-head">
      ...
      <h2 id="projectDetailTitle">${esc(p.name)}</h2>
      <p>${esc(p.note||'ยังไม่ได้ระบุรายละเอียดเพิ่มเติม')}</p>
    </header>
    <div class="project-detail-body">
      <section class="project-metric-grid">
        <div><span>ไฟล์ Progress</span><strong>${m.files}</strong></div>
        <div><span>ปีงาน</span><strong>${m.years.length}</strong></div>
        <div><span>แปลงก่อนรวมซ้ำ</span><strong>${m.plots}</strong></div>
        <div><span>เอกสารแนบ</span><strong>${p.attachments.length}</strong></div>
      </section>
      <section class="project-detail-section">
        <div class="project-section-head"><div><h3>Progress รายปี</h3><p>Dashboard ใช้มาตรฐานเดียวกันทุกโครงการและแยกตามปีงาน</p></div><button class="btn ghost compact" type="button" data-detail-action="add-excel">＋ เพิ่ม Excel</button></div>
        <div class="project-year-tags">${yearTags}</div>
        <div class="project-data-files">${dataFiles}</div>
      </section>
      <section class="project-detail-section">
        <div class="project-section-head"><div><h3>เอกสารแนบโครงการ</h3><p>แนบสัญญา แผนงาน รายงาน หรือเอกสารประกอบของโครงการนี้</p></div><button class="btn ghost compact" type="button" data-detail-action="add-attachment">＋ แนบเอกสาร</button></div>
        <div class="attachment-list">${attachments}</div>
        <div class="attachment-note">เอกสารอยู่ในแท็บนี้เท่านั้น ไม่ถูกอัปโหลดขึ้น server และจะหายเมื่อ Refresh หรือปิดแท็บ</div>
      </section>
    </div>
    <footer class="project-detail-actions">
      <div><button class="btn ghost" type="button" data-detail-action="edit">แก้ไขข้อมูลโครงการ</button><button class="btn danger-outline" type="button" data-detail-action="delete">ลบโครงการ</button></div>
      <button class="btn project-primary" type="button" data-detail-action="open-dashboard">เปิด Dashboard โครงการนี้</button>
    </footer>`;
}
function openProjectDetail(id){
  const p=projectById(id);if(!p)return;
  S.detailId=id;ensureFrame(p);renderProjectDetail();$('projectDetailModal').hidden=false;bodyLock();renderSelector();
}
function closeProjectDetail(){$('projectDetailModal').hidden=true;S.detailId=null;bodyLock();renderSelector()}

function sendExcelFiles(p,files){
  const list=[...files];if(!p||!list.length)return;
  const frame=ensureFrame(p);
  const deliver=()=>{
    try{
      const input=frame.contentDocument?.getElementById('fileInput');
      if(!input)throw new Error('ไม่พบช่องเลือกไฟล์ใน Dashboard');
      const transfer=new DataTransfer();list.forEach(file=>transfer.items.add(file));input.files=transfer.files;input.dispatchEvent(new Event('change',{bubbles:true}));
      selectProject(p.id);setTimeout(()=>updateProjectStatus(p),600);
    }catch(err){console.error(err);selectProject(p.id);alert('กรุณากด “เพิ่มไฟล์” ใน Dashboard เพื่อเลือก Excel อีกครั้ง')}
  };
  if(frame.contentDocument?.readyState==='complete')deliver();else frame.addEventListener('load',deliver,{once:true});
}
function addAttachments(p,files){
  if(!p)return;
  [...files].forEach(file=>{
    const duplicate=p.attachments.some(a=>a.name===file.name&&a.size===file.size&&a.lastModified===file.lastModified);if(duplicate)return;
    p.attachments.push({id:uid('attachment'),name:file.name,size:file.size,type:file.type,lastModified:file.lastModified,url:URL.createObjectURL(file),addedAtLabel:new Intl.DateTimeFormat('th-TH',{dateStyle:'medium',timeStyle:'short'}).format(new Date())});
  });
  renderProjectDetail();renderCards();if(p.id===S.activeId)renderWorkspaceHead();
}
...
function openProjectModal(mode,id=null){
  ...
  $('projectModalTitle').textContent=p?'แก้ไขข้อมูลโครงการ':'สร้างโครงการใหม่';$('projectName').value=p?.name||'';$('projectNote').value=p?.note||'';$('projectFormError').textContent='';$('projectModal').hidden=false;bodyLock();setTimeout(()=>$('projectName').focus(),0);
}
...
function saveProject(){
  ...
  if(S.editingId){
    const p=projectById(S.editingId);if(!p)return;p.name=name;p.note=note;
    if(p.iframe){p.iframe.title=`Dashboard: ${name}`;try{p.iframe.contentDocument.title=`${name} · Work Progress Monitor`}catch{}}
    persistProjects();closeProjectModal();renderAll();if(p.id===S.detailId)renderProjectDetail();
  }else{
    const p=createRuntimeProject({id:uid('project'),name,note,createdAt:Date.now()});p.status='ยังไม่มีไฟล์ Progress';S.projects.push(p);persistProjects();closeProjectModal();renderAll();openProjectDetail(p.id);
  }
}
function deleteProject(id){
  const p=projectById(id);if(!p)return;
  const extra=(p.metrics?.files||p.attachments.length)?'\n\nไฟล์ Excel และเอกสารที่เลือกไว้จะถูกนำออกจากหน้าเว็บด้วย':'';
  if(!confirm(`ลบโครงการ “${p.name}” หรือไม่?${extra}`))return;
  p.attachments.forEach(a=>URL.revokeObjectURL(a.url));p.observer?.disconnect();p.resizeObserver?.disconnect();clearTimeout(p.statusTimer);p.iframe?.remove();
  S.projects=S.projects.filter(x=>x.id!==id);if(S.detailId===id)closeProjectDetail();if(S.activeId===id){S.activeId=null;S.mode='hub';document.title='Work Progress Monitor · Project Hub'}persistProjects();renderAll();setMode(S.activeId?'workspace':'hub');
}
...
function handleProjectAction(target){
  ...
  else if(action==='open'&&id)openProjectDetail(id);
  else if(action==='edit'&&id)openProjectModal('edit',id);
  else if(action==='delete'&&id)deleteProject(id);
}
function handleDetailAction(target){
  const button=target.closest('[data-detail-action]');if(!button)return;
  const p=detailProject();if(!p)return;
  const action=button.dataset.detailAction;
  if(action==='open-dashboard')selectProject(p.id);
  else if(action==='add-excel'){$('projectExcelInput').value='';$('projectExcelInput').click()}
  else if(action==='add-attachment'){$('projectAttachmentInput').value='';$('projectAttachmentInput').click()}
  else if(action==='remove-attachment')removeAttachment(p,button.dataset.attachment);
  else if(action==='edit')openProjectModal('edit',p.id);
...
$('projectInfoBtn').onclick=()=>{if(S.activeId)openProjectDetail(S.activeId)};
...
$('projectDetailContent').addEventListener('click',e=>handleDetailAction(e.target));
$('projectDetailClose').onclick=closeProjectDetail;
$('projectDetailModal').addEventListener('click',e=>{if(e.target===$('projectDetailModal'))closeProjectDetail()});
$('projectExcelInput').onchange=e=>{const p=detailProject();if(p)sendExcelFiles(p,e.target.files)};
$('projectAttachmentInput').onchange=e=>{const p=detailProject();if(p)addAttachments(p,e.target.files)};
...
window.addEventListener('beforeunload',()=>S.projects.forEach(p=>p.attachments.forEach(a=>URL.revokeObjectURL(a.url))));
